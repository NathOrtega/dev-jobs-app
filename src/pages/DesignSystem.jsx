...
import {
	...
	titlesStyles,
	paragraphStyles,
} from "../components/designSystem/constants";
import TypographySampler, {
	FontFamilySampler,
} from "../components/designSystem/TypographySampler";
	...
	return (
		<React.Fragment>
			...
			<TwoColumnsSection
				numeration="2"
				title="Typography"
				introInfo={<FontFamilySampler fontFamily="Kumbh Sans" />}
			>
				<Column title="Headings">
					{titlesStyles.map(
						({ type, fontSize, lineHeight, sampleText }, index) => {
							return (
								<TypographySampler
									key={index}
									type={type}
			...
			<TwoColumnsSection numeration="4" title="Errors & Loader">
				<Row title="General error">
					<Button variant="primary" onClick={throwError}>
						Error
					</Button>
				</Row>
				<Row title="404 error">
					<Link to="/url-that-does-not-exist">
						<Button variant="primary">404</Button>
					</Link>
				</Row>
				<Row title="Loading">
